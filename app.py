...
@app.route("/facedetection_req",methods=["GET","POST"])
def facedetection_req():
    if request.method == "POST":


        if request.form["facedetection_type"] == "Detect":
            path = FaceDetection.draw_faces()
        # elif request.form["facedetection_type"] == "Otsu":
        #     path = facedetection.otsuThreshold()
        # elif request.form["facedetection_type"] == "spectral_Otsu":
        #     path = facedetection.segment_image_otsu(n=number_slices)
        # elif request.form["facedetection_type"] == "spectral_kmean":
        #     path = facedetection.apply_spectral_kmean(num_clusters=number_slices)
        print(path)
    return json.dumps({1:path})
...
def facedetection():
    return render_template("facedetection.html", img = session["uploaded_img"])
